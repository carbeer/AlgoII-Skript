...
In gerichteten Graphen sind \term{starke Zusammenhangskomponenten}\index{Zusammenhangskomponente!stark} (SCCs) Teilgraphen \( G \subseteq H \), in denen ebenfalls gilt: für jedes Knotenpaar \( u,v \in G \) gibt es einen \( u \)-\( v \)-Pfad \emph{und} einen \( v \)-\( u \)-Pfad.
...
\begin{figure}[H]
	\includegraphics[width=0.6\textwidth]{dfs_numbering}
...
\begin{enumerate}
  ...
  \item Jede Komponente besitzt einen Repräsentanten, welcher die offenen Komponenten bezüglich ihrer \code{dfsNum} partitioniert.
\end{enumerate}
